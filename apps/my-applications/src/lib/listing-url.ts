/**
 * Build `/job-search?` query URLs for the job listing, preserving filters.
 */
export const JOB_LISTING_PATH = "/job-search";
...
export function listingUrl(
  ...
  const s = u.toString();
  return s ? `${JOB_LISTING_PATH}?${s}` : JOB_LISTING_PATH;
}
